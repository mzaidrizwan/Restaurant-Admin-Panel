...
function show(e) {
    ...
    let inp = e && e.target ? e.target.value.toLowerCase().trim() : "";
    ...
    // WHEN INPUT IN SEARCH
    data.forEach((element, i) => {
        let selectedSearchFilter = document.getElementById("searchBy").value;
        let nameLower;

        if (selectedSearchFilter === "hotelName") {
            nameLower = element.hotelName.toLowerCase();
        } else if (selectedSearchFilter === "proprietorPhone") {
            nameLower = element.proprietorPhone.toLowerCase();
        } else if (selectedSearchFilter === "locationSelect") {
            nameLower = element.locationSelect.toLowerCase();
        }

        let found = false;
        for (let i = 0; i <= nameLower.length - inp.length; i++) {
            let cut = nameLower.slice(i, i + inp.length);
            if (cut === inp) {
                found = true;
                break;
            }
